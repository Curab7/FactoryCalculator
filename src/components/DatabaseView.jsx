      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative">
            <label className="text-[10px] text-slate-400 font-bold uppercase block">当前配方组</label>
            <select
              value={currentGroupName}
              onChange={(e) => updateGroups(recipeGroups, e.target.value)}
              className="bg-transparent font-bold text-slate-800 text-lg outline-none cursor-pointer pr-6 appearance-none hover:text-blue-600"
            >
            ...
            <span className="absolute right-0 bottom-1.5 pointer-events-none text-slate-400">⌄</span>
          </div>
        </div>
        ...
        <div className="flex gap-2 w-full md:w-auto overflow-x-auto">
          <button
            ...
            className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 transition whitespace-nowrap"
          >
          ...
          <label className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 transition cursor-pointer whitespace-nowrap">
            <Upload size={16} /> 导入
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition whitespace-nowrap"
          >
            <Trash2 size={16} /> 清空
          </button>
      ...
      {isGroupModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm">
            <h3 className="text-lg font-bold mb-4">新建配方组</h3>
            <input
              autoFocus
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setIsGroupModalOpen(false)} className="px-4 py-2 text-slate-500 hover:bg-slate-100 rounded-lg">
                取消
              </button>
              <button onClick={handleCreateGroup} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                创建
              </button>
            </div>
          </div>
            ...
            <div className="flex items-start gap-4 mb-4">
              <div
                className={`p-3 rounded-full ${
                  modalConfig.type === 'alert' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'
                }`}
              >
              ...
              <div>
                ...
                <p className="text-slate-600 text-sm mt-1 leading-relaxed">{modalConfig.message}</p>
              </div>
            </div>
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className={`px-4 py-2 text-white rounded-lg text-sm font-medium transition shadow-sm ${
                  modalConfig.type === 'alert' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-red-600 hover:bg-red-700'
                }`}
              >
          ...
          <div className="space-y-4">
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
              <div className="flex justify-between items-center mb-2">
                <label className="text-xs font-bold text-slate-500 uppercase">输入原料</label>
                <button
              ...
              {newRecipe.inputs.map((input, idx) => (
                <div key={idx} className="flex gap-2 mb-2">
                  <input
                    placeholder="名称"
                    className="flex-1 px-2 py-1 text-sm border rounded focus:border-blue-500 outline-none"
                    value={input.item}
                  ...
                  <input
                    type="number"
                    placeholder="#"
                    className="w-16 px-2 py-1 text-sm border rounded focus:border-blue-500 outline-none"
                    value={input.amount}
                    onChange={(e) => updateInput(idx, 'amount', e.target.value)}
                  />
                  {newRecipe.inputs.length > 1 && (
                    <button onClick={() => removeInput(idx)} className="text-red-400 hover:text-red-600">
                      <X size={14} />
                    </button>
                  )}
                </div>
            ...
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
              <div className="flex justify-between items-center mb-2">
                <label className="text-xs font-bold text-slate-500 uppercase">输出产品</label>
                <button
              ...
              {newRecipe.outputs.map((output, idx) => (
                <div key={idx} className="flex gap-2 mb-2">
                  ...
                  {newRecipe.outputs.length > 1 && (
                    <button onClick={() => removeOutput(idx)} className="text-red-400 hover:text-red-600">
                      <X size={14} />
                    </button>
                  )}
                </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-slate-500 mb-1">生产设施</label>
                <input
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-1">耗时 (秒)</label>
                <input
                  type="number"
                  className="w-full px-3 py-2 text-sm border rounded outline-none focus:border-blue-500"
            ...
            <button
              ...
              className="w-full bg-blue-600 text-white py-2.5 rounded-lg hover:bg-blue-700 transition shadow-md font-bold text-sm"
            >
              保存配方
            </button>
          </div>
          ...
          <div className="grid gap-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
            {filteredRecipes.length > 0 ? (
              filteredRecipes.map((recipe) => (
                <div
                  key={recipe.id}
                  className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition flex justify-between items-center group"
                >
                  <div className="flex-1">
                    <div className="flex items-center flex-wrap gap-2 text-sm">
                      <div className="flex items-center gap-1">
                        {recipe.inputs.map((i, idx) => (
                          <span key={idx} className="bg-slate-100 text-slate-600 px-2 py-1 rounded">
                      ...
                      <div className="flex items-center gap-1">
                        {recipe.outputs.map((o, idx) => (
                          <span
                            key={idx}
                            className="bg-green-50 text-green-700 border border-green-100 px-2 py-1 rounded font-medium"
                          >
